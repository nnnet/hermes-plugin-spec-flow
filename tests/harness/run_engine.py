...
class Engine:
    def __init__(self, tools: Any, sink: Optional[Any] = None,
        ...
        if sink is None:
            ...
            self.sink = sink
        # workspace: explicit Workspace / path, else env-driven default (off
        # unless SPEC_FLOW_RUN_WORKSPACE is set).
        if workspace is None:
            self.workspace = Workspace.from_env()
    ...
    def _visit(self, node: dict, depth: int, contract_ctx: Optional[dict], phase: str,
        ...
        if verdict == "branch":
            ...
            if contract_here:
                ...
                self.workspace.contract(str(CONTRACTS / contract_here.get("fixed", contract_here["artifact"])),
    ...
    def _leaf_pipeline(self, node: dict, contract_ctx: Optional[dict],
        ...
        nid = node["id"]
        title = node.get("title", nid)
        # materialise the leaf spec/plan + code & test scaffolds
