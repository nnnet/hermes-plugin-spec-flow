#!/usr/bin/env python3
"""Audit STAGE 23 (S23.1-S23.3): the narrow hand-written OpenAPI interface
...
# --- S23.1: interface comes from the compiled document, gaps included --------
...
def test_media_gap_contract_is_named_not_silently_passed(tmp_path):
    """S23.1: a success response with NO media is an honest compiler gap;
...
def test_empty_schema_contract_is_named_not_silently_passed(tmp_path):
    """S23.2: an empty ``{}`` success schema is a body-shape gap the compiler
...
# --- S23.3: honest recorded fields still diff exactly as before ---------------
...
def test_recorded_fields_still_diff_exactly(tmp_path):
    """S23.3: for a fully-recorded contract the compiled-document diff keeps the
