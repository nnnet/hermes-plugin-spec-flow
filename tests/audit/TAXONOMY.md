...
## STAGE 23 — Narrow OpenAPI interface diff built ON the compiled machine document (node K4, `test_openapi_diff_compiled.py`)
- S23.1 THE DIFF'S INTERFACE IS THE COMPILER'S DOCUMENT, NOT A PRIVATE WALK:
...
- S23.2 A COMPILER-NAMED GAP IS DRIFT, NEVER A FALSE GREEN: a success response
...
- S23.3 REAL FIELD DRIFT IS UNWEAKENED: for a fully-recorded contract the
...
- S23 NOTE — LIBRARY vs HAND-ROLL DECISION: no third-party OpenAPI *diff*
