...
graph:
  ...
  - {id: M1, needs: [I3, K3],         parallel: "dash",    status: "[~]", files: [tests/lib/live_dashboard.py]}
...
### G2 `research-order-race` — планировщик волн теряет порядок research→impl
...
- приёмка: grep не находит прямых читателей сырых датумов в горячем пути;
  дубль устранён
- заметки: ГОТОВ — Stage S26, коммит 2139d6a. Замыкает переворот. Три
