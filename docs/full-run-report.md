# spec-flow run — отчёт по логам (footprints + методологический аудит)
...
| | Правило | Где | Почему | Как исправить |
|---|---|---|---|---|
...
## Шаги на снегу (детализация ≤ 2)
```
TICK │ ACTOR · SKILL · [TASK] action → result
── decompose ──
t 1 │ 🧩 spec-decomposer · spec-flow-decompose · [L0] leaf_check → branch  «modules 3 > 1»
t 2 │ 🧩 spec-decomposer · spec-flow-decompose · [svc] leaf_check → leaf  «within all thresholds»
── implement ──
t 3 │ 🛠️ implementer · spec-implement · [svc:impl] design then code (NO TDD, NO review)
t 4 │ 🛠️ implementer · spec-implement · [svc:impl] contract_check vs frozen L2 → drift  «type_mismatch count integer vs string»
t 5 │ 🛠️ implementer · spec-implement · [svc:impl] edited code to match (silent)  «no drift-gate, no respec»
── decompose ──
t 6 │ 🧩 spec-decomposer · spec-flow-decompose · [subsys] leaf_check → branch  «modules 2 > 1; 1 open decision(s) — resolve before leafing»
── implement ──
t 7 │ 🛠️ implementer · spec-implement · [subsys_leaf:impl] implement child directly  «parent never leaf-gated this child»
── integrate ──
t 8 │ ✅ verifier · spec-integrate · [L0:integrate] L0 integrate done = project COMPLETE  «declared done»
```
